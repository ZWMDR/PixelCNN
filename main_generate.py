...
# Load model
model, data_loader = load_model(args.model_folder, model_version=args.model_version)
...
if args.gen_type == 'generation' or args.gen_type == 'uncertainty':
    ...
    for i in range(num_img):
        for j in range(len(mask_descriptors)):
            ...
            save_image(output["orig_img"].float() / (256 - 1.), directory + "/source_{}_{}.png".format(i, j))
            ...
            save_image(output["samples"].float().mean(dim=0) / (256 - 1.), directory + '/mean_samples_{}_{}.png'.format(i, j))
            ...
            if args.gen_type == 'generation':
                save_image(output["samples"].float() / (256 - 1.), directory + "/samples_{}_{}.png".format(i, j), nrow=args.num_per_row, pad_value=1)
            elif args.gen_type == 'uncertainty':
                sorted_samples, log_likelihoods = uncertainty_plot(output["samples"], output["log_probs"])
                save_image(sorted_samples.float() / (256 - 1.), directory + "/sorted_samples_{}_{}.png".format(i, j), nrow=args.num_per_row, pad_value=1)
    ...
    if 256 != 2:
        raise(RuntimeError("Logits generation only works for models with 2 colors. Current model has {} colors.".format(256)))
    # Generate images with model
    outputs = generate_images(model, batch, mask_descriptors,
